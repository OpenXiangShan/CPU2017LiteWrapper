...
mkdir -p cpu2017_build.${toolchain_name}
...
find . -name "build" | while read build_dir; do
    ...
    if [ -n "$target_file" ]; then
        echo copying $target_file to cpu2017_build.${toolchain_name}/$new_name
        cp "$target_file" "cpu2017_build.${toolchain_name}/$new_name"
    fi
done
